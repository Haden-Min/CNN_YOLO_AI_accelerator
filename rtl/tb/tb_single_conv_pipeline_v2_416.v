...
module tb_single_conv_pipeline_v2_416;
    ...
    localparam INPUT_ADDR_WIDTH = 18;
    localparam WEIGHT_ADDR_WIDTH = 4;
    localparam BIAS_ADDR_WIDTH = 1;
    localparam OUTPUT_ADDR_WIDTH = 18;
    localparam TIMEOUT_CYCLES = 1200000;
    ...
    task ps_write_input_row;
        ...
        integer flat_idx;
        begin
            if ((row_idx < K_H) || ((row_idx % 32) == 0) || (row_idx == IN_H - 1)) begin
    ...
    task ps_wait_done_and_check_live_writes;
        begin
            ...
            while (done_seen == 0 && cycles < TIMEOUT_CYCLES) begin
                @(posedge clk);
                #1;
                cycles = cycles + 1;
                ...
                if (output_we_o) begin
                    ...
                    if ((write_count % PROGRESS_STEP) == 0) begin
                        $display("[PROGRESS] live writes=%0d/%0d cycles=%0d",
                                 write_count, OUTPUT_SIZE, cycles);
                    end
                end
        ...
        begin
            ...
            for (i = 0; i < OUTPUT_SIZE; i = i + 1) begin
                ...
                if (((i + 1) % PROGRESS_STEP) == 0) begin
                    $display("[PROGRESS] readback=%0d/%0d", i + 1, OUTPUT_SIZE);
                end
    ...
    initial begin
        ...
        if (mismatch_count != 0 || readback_mismatch_count != 0) begin
            ...
            $display("PASS: tb_single_conv_pipeline_v2_416 writes=%0d cycles=%0d",
                     write_count, cycles);
        end
